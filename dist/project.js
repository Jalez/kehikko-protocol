 ...
 */
export function kehikkoDir(projectPath) {
 ...
 */
export function kehikkoFile(projectPath, name) {
    if (!DATA_FILE.test(name)) {
        throw new Error(`"${name}" is not a name a module may give its own file. A file under ${KEHIKKO_DIR}/ is spelled with `
 ...
 * than wrong.
 */
export function ignoresKehikko(gitignore) {
 ...
 */
export function withKehikkoIgnored(gitignore) {
    if (ignoresKehikko(gitignore))
        return gitignore;
    if (!gitignore.trim())
        return KEHIKKO_IGNORE;
