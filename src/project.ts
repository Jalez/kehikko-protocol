 ...
 */
export const KEHIKKO_DIR = '.kehikko'
 ...
 */
export function kehikkoDir(projectPath: string | null | undefined): string | null {
  if (typeof projectPath !== 'string') return null
  const path = projectPath.trim()
  if (!path) return null
  return joined(path, KEHIKKO_DIR)
}
 ...
 */
export function kehikkoFile(projectPath: string | null | undefined, name: string): string | null {
  if (!DATA_FILE.test(name)) {
    throw new Error(
      `"${name}" is not a name a module may give its own file. A file under ${KEHIKKO_DIR}/ is spelled with `
        + 'lowercase letters, digits and dashes, and is a constant in the module that owns it — never a string that '
        + 'arrived from anywhere.',
    )
  }
  const dir = kehikkoDir(projectPath)
 ...
 */
export const KEHIKKO_IGNORE = `# Modules on a roadmap host keep this project's data here — checklists, notes,
 ...
 * than wrong.
 */
export function ignoresKehikko(gitignore: string): boolean {
  return gitignore.split(/\r?\n/).some((line) => {
    ...
    return rule === KEHIKKO_DIR || rule.endsWith(`/${KEHIKKO_DIR}`)
  })
}
 ...
 */
export function withKehikkoIgnored(gitignore: string): string {
  if (ignoresKehikko(gitignore)) return gitignore
  if (!gitignore.trim()) return KEHIKKO_IGNORE
